...
(function() {
  ...
  var hideOldEmerg = function() {
    ...
    st.textContent =
      '#hc-emerg, #chatUrg, #hcChatFab, #hcChatWin, .chat-urg, .chat-urg-fab, .chat-urg-head-bubble, [aria-label="Zone d\'intervention plomberie"], [aria-label="Zone d\'intervention chauffage"], [aria-label="Zone d\'intervention électricité"], [aria-label="Zone d\'intervention serrurerie"], [aria-label="Zone d\'intervention vitrerie"], [aria-label="Zone d\'intervention menuiserie"], [aria-label="Zone d\'intervention volets"], [aria-label="Zone d\'intervention rénovation"], [aria-label="Zone d\'intervention PMR"], section[aria-label^="Zone d\'intervention"], .m-zone-compact { display: none !important; visibility: hidden !important; pointer-events: none !important; }' +
      // Fix dropdown menus
...
(function(){
  ...
  // Activation : ?edit=1 dans URL OU cookie hc_edit=1
  ...
  var GH_OWNER = "floriian62500-code", GH_REPO = "site-help-confort", GH_BRANCH = "staging";
  var SUPA_URL = "https://btcbjwqiivhpwoszomhg.supabase.co";

  function injectStyle(){
    if (document.getElementById("hc-wyz-style")) return;
    var s = document.createElement("style");
    s.id = "hc-wyz-style";
    s.textContent = [
      ".hc-wyz-wrap{position:relative;display:inline-block;isolation:isolate}",
      ".hc-wyz-wrap:hover .hc-wyz-overlay{opacity:1;pointer-events:auto}",
      ".hc-wyz-overlay{position:absolute;top:0;left:0;right:0;bottom:0;background:rgba(13,160,207,.18);border:2px dashed #0DA0CF;display:flex;align-items:center;justify-content:center;opacity:0;transition:all .15s;pointer-events:none;cursor:pointer;z-index:1}",
      ".hc-wyz-btn{background:#0DA0CF;color:#fff;border:none;border-radius:8px;padding:8px 14px;font-weight:800;font-size:.78rem;font-family:Inter,system-ui,sans-serif;cursor:pointer;box-shadow:0 4px 12px rgba(13,160,207,.4)}",
      "#hc-wyz-bar{position:fixed;top:0;left:0;right:0;background:#0DA0CF;color:#fff;padding:8px 16px;font-family:Inter,system-ui,sans-serif;font-size:.84rem;font-weight:700;z-index:99996;text-align:center;display:flex;align-items:center;justify-content:center;gap:14px}",
      "#hc-wyz-bar button{background:rgba(255,255,255,.2);border:none;color:#fff;padding:4px 10px;border-radius:5px;font-size:.74rem;font-weight:700;cursor:pointer}",
      "#hc-wyz-bar button:hover{background:rgba(255,255,255,.3)}",
      "body.hc-wyz-on{padding-top:36px !important}",
      "#hc-wyz-modal{position:fixed;inset:0;background:rgba(10,20,40,.6);z-index:99999;display:none;align-items:center;justify-content:center;padding:20px}",
      "#hc-wyz-modal.open{display:flex}",
      "#hc-wyz-modal .panel{background:#fff;border-radius:14px;padding:24px;max-width:520px;width:100%;font-family:Inter,system-ui,sans-serif}",
      "#hc-wyz-modal h3{margin:0 0 4px;font-size:1.04rem;font-weight:800;color:#0A1428}",
      "#hc-wyz-modal .meta{font-size:.78rem;color:#94a3b8;font-family:ui-monospace,monospace;word-break:break-all;margin-bottom:18px}",
      "#hc-wyz-modal .preview-row{display:flex;gap:14px;margin-bottom:18px}",
      "#hc-wyz-modal .preview-row > div{flex:1;border:1px solid #E5EDF3;border-radius:8px;padding:10px;text-align:center}",
      "#hc-wyz-modal .preview-row img{max-width:100%;max-height:120px;object-fit:contain}",
      "#hc-wyz-modal .preview-row .label{font-size:.7rem;font-weight:700;text-transform:uppercase;color:#64748b;margin-bottom:8px}",
      "#hc-wyz-modal .dropzone{border:2px dashed #CBD5E1;border-radius:10px;padding:30px;text-align:center;color:#64748b;font-size:.86rem;cursor:pointer;transition:all .15s}",
      "#hc-wyz-modal .dropzone:hover, #hc-wyz-modal .dropzone.over{border-color:#0DA0CF;background:#EFF8FE;color:#0DA0CF}",
      "#hc-wyz-modal .actions{display:flex;gap:8px;margin-top:16px}",
      "#hc-wyz-modal .actions button{flex:1;border:1px solid #E5EDF3;background:#fff;border-radius:8px;padding:10px;font-weight:700;font-size:.86rem;cursor:pointer;font-family:inherit}",
      "#hc-wyz-modal .actions .cancel{color:#475569}",
      "#hc-wyz-modal .actions .upload{background:#22C55E;color:#fff;border-color:#22C55E}",
      "#hc-wyz-modal .actions .upload:disabled{background:#CBD5E1;border-color:#CBD5E1;cursor:not-allowed}"
    ].join("");
    document.head.appendChild(s);
  }

  function getToken(){ return localStorage.getItem("hc_gh_token") || ""; }
  function setToken(t){ localStorage.setItem("hc_gh_token", t); }

  function relativePath(src){
    if (!src) return null;
    try {
      var u = new URL(src, location.origin);
      // Ne traite que les images du même domaine staging dans /images/* ou racine
      if (u.hostname !== location.hostname) return null;
      var p = u.pathname.replace(/^\//, "");
      if (!/\.(webp|png|jpg|jpeg|svg|gif|avif)$/i.test(p)) return null;
      return p;
    } catch(e) { return null; }
  }

  function injectBar(){
    if (document.getElementById("hc-wyz-bar")) return;
    document.body.classList.add("hc-wyz-on");
    var bar = document.createElement("div");
    bar.id = "hc-wyz-bar";
    bar.innerHTML = "\ud83c\udfa8 MODE \u00c9DITION ACTIF \u00b7 Click sur une image pour la remplacer <button id=\"hc-wyz-quit\">Quitter</button> <button id=\"hc-wyz-token\">PAT GitHub</button>";
    document.body.insertBefore(bar, document.body.firstChild);
    document.getElementById("hc-wyz-quit").onclick = function(){
      document.cookie = "hc_edit=; path=/; max-age=0";
      location.reload();
    };
    document.getElementById("hc-wyz-token").onclick = function(){
      var t = prompt("Token GitHub PAT (ghp_...) :", getToken());
      if (t !== null) setToken(t.trim());
    };
  }

  function wrapImg(img){
    if (img.dataset.hcWyz === "1") return;
    var p = relativePath(img.src);
    if (!p) return;
    if (img.parentElement && img.parentElement.classList.contains("hc-wyz-wrap")) return;
    img.dataset.hcWyz = "1";
    var wrap = document.createElement("span");
    wrap.className = "hc-wyz-wrap";
    wrap.style.display = (getComputedStyle(img).display || "inline") === "block" ? "block" : "inline-block";
    var overlay = document.createElement("div");
    overlay.className = "hc-wyz-overlay";
    overlay.innerHTML = "<button class=\"hc-wyz-btn\">\u270f\ufe0f Remplacer</button>";
    overlay.onclick = function(e){ e.preventDefault(); e.stopPropagation(); openModal(img, p); };
  ...
  function injectModal(){
    if (document.getElementById("hc-wyz-modal")) return;
    var m = document.createElement("div");
    m.id = "hc-wyz-modal";
    m.innerHTML = "<div class=\"panel\">"+
      "<h3>Remplacer cette image</h3>"+
      "<div class=\"meta\" id=\"hc-wyz-path\"></div>"+
      "<div class=\"preview-row\">"+
        "<div><div class=\"label\">Actuelle</div><img id=\"hc-wyz-current\" alt=\"\"></div>"+
        "<div><div class=\"label\">Nouvelle</div><img id=\"hc-wyz-new\" alt=\"\"><span id=\"hc-wyz-new-empty\" style=\"color:#94a3b8;font-size:.78rem\">Drop fichier</span></div>"+
      "</div>"+
      "<label class=\"dropzone\" id=\"hc-wyz-drop\">"+
        "<div>\ud83d\udcce Drag-drop OU click pour choisir un fichier</div>"+
        "<input type=\"file\" accept=\"image/*,.svg\" style=\"display:none\" id=\"hc-wyz-file\">"+
      "</label>"+
      "<div class=\"actions\">"+
        "<button class=\"cancel\" id=\"hc-wyz-cancel\">Annuler</button>"+
        "<button class=\"upload\" id=\"hc-wyz-upload\" disabled>Pousser sur staging</button>"+
      "</div>"+
    "</div>";
    document.body.appendChild(m);
    m.addEventListener("click", function(e){ if (e.target === m) closeModal(); });
    document.getElementById("hc-wyz-cancel").onclick = closeModal;
    var input = document.getElementById("hc-wyz-file");
    var drop = document.getElementById("hc-wyz-drop");
  ...
  var currentImg = null, currentPath = null, currentFile = null, currentB64 = null;

  function openModal(img, path){
    injectModal();
    currentImg = img; currentPath = path; currentFile = null; currentB64 = null;
    document.getElementById("hc-wyz-path").textContent = path;
    document.getElementById("hc-wyz-current").src = img.src;
    document.getElementById("hc-wyz-new").src = "";
    document.getElementById("hc-wyz-new").style.display = "none";
    document.getElementById("hc-wyz-new-empty").style.display = "inline";
    document.getElementById("hc-wyz-upload").disabled = true;
    document.getElementById("hc-wyz-modal").classList.add("open");
  }
  function closeModal(){
    var m = document.getElementById("hc-wyz-modal");
  ...
  function handleFile(file){
    ...
    r.onload = function(){
      ...
      var prev = document.getElementById("hc-wyz-new");
      prev.src = dataUrl;
      prev.style.display = "inline";
      document.getElementById("hc-wyz-new-empty").style.display = "none";
      document.getElementById("hc-wyz-upload").disabled = false;
    };
  ...
  function upload(){
    var token = getToken();
    if (!token){ alert("Renseigne ton PAT GitHub via le bouton en haut"); return; }
    if (!currentB64 || !currentPath) return;
    var btn = document.getElementById("hc-wyz-upload");
    ...
    fetch(SUPA_URL + "/functions/v1/gh-push-inline", {
      ...
      body: JSON.stringify({
        ...
        message: "feat(wysiwyg): replace " + currentPath,
        files: [{ path: currentPath, content_b64: currentB64 }]
      })
    }).then(function(r){return r.json();}).then(function(d){
      if (d.success){
        alert("\u2705 Push\u00e9 sur staging ! Recharge la page dans 2 min");
        closeModal();
      } else {
        alert("\u274c " + (d.error || JSON.stringify(d)));
        btn.disabled = false; btn.textContent = "Pousser sur staging";
      }
    }).catch(function(e){
  ...
  function init(){
    injectStyle();
    injectBar();
    // Wrap toutes les images existantes
    document.querySelectorAll("img").forEach(wrapImg);
    // Wrap les images ajoutées dynamiquement
        ...
        muts.forEach(function(m){
          m.addedNodes.forEach(function(n){
            if (n.nodeType === 1){
              ...
              else if (n.querySelectorAll) n.querySelectorAll("img").forEach(wrapImg);
            }
          });
        });
      });
    ...
    // Click sur le bouton upload (delegate)
    document.addEventListener("click", function(e){
      if (e.target && e.target.id === "hc-wyz-upload") upload();
    });
  }
