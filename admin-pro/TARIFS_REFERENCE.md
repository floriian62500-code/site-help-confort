# 💰 Tarifs HELP Confort — Source de vérité

> **Source** : `Base_produits_2026-05-14_11-11.xlsx` onglet **BAREME AGENCE** uniquement
> **Validé par** : Florian (florian.dhaillecourt@helpconfort.com)
> **Dernière mise à jour** : 19 septembre 2026 (poêles et inserts EPB / EPG)

Tous les tarifs affichés sur le site doivent être croisés avec ce document. Tout montant absent → "sur devis".

---

## 🕐 Tarif horaire universel (tous métiers)

| | Prix HT | Prix TTC |
|--|--------|----------|
| **Main d'œuvre tous métiers** (Saint-Omer) | 53 €/h | **58 €/h** |
| **Main d'œuvre tous métiers** (Dunkerque) | 55 €/h | **60,50 €/h** |
| **Main d'œuvre urgence (intervention <1h)** | 63 €/h | **69 €/h** |
| **Main d'œuvre week-end** | 80 €/h | **88 €/h** |
| **Déplacement audomarois** | 45 € | **50 €** |
| **Déplacement Dunkerque** | 48 € | **52,80 €** |
| **Frais de dossier** (systématique) | 5 € | **5,50 €** |
| **Traitement des déchets** (% obligatoire) | **1 % du HT** | **1 % du HT** |

### 🧮 Base d'un devis « tout compris »
Tout devis HELP Confort comprend systématiquement :
- Main d'œuvre (53 €/h HT)
- Déplacement (45 € HT)
- Frais de dossier (5 € HT)
- Traitement déchets = **1 % du HT total** (calculé automatiquement après les autres lignes)
- = **103 € HT** + 1,03 € déchets = **104,03 € HT base** (≈ **114,40 € TTC**) auxquels on ajoute la fourniture éventuelle + MO supplémentaire si > 1h.

> ⚠️ **Règle métier** (depuis 2026-05-15 — décision Florian) : le traitement des déchets n'est plus un montant fixe mais **1 % du montant HT du forfait commandé**. Sur un forfait à 1 000 € HT → 10 € de déchets. Sur un mécanisme Nicoll à 170 € HT → 1,70 € de déchets. Calculé en bout de chaîne et affiché en ligne séparée sur le devis.

---

## 🔧 Plomberie (5 forfaits)

| Prestation | Tarif TTC | Référence |
|------------|-----------|-----------|
| Recherche de fuite visuelle | **148 €** | FRFV |
| Recherche de fuite technique (caméra/gaz/acoustique) | **383 €** | FRFT |
| Désengorgement canalisation | **165 €** | — |
| Mécanisme chasse d'eau **Nicoll** (tout compris) | **187 €** | devis 15/05 |
| Mécanisme chasse d'eau **Geberit** (tout compris, WC sol) | **203 €** | devis 15/05 |
| Détartrage installation sanitaire | **237 €** | — |

### 💧 Chauffe-eau Atlantic (tout compris : fourniture + pose 2 techniciens + accessoires + déchets + déplacement + frais)
| Volume | Pose | Gamme Éco (blindé) | Gamme Stéatite |
|--------|------|---------------------|----------------|
| **100 L** | mural / trépied | **817 €** | **887 €** |
| **150 L** | mural / trépied | **884 €** | **961 €** |
| **200 L** | au sol | **1 332 €** | **~1 456 €** (à vérifier) |

Accessoires inclus dans chaque pose : trépied · groupe de sécurité SFR 3/4 · siphon · petite fourniture. Garantie 5 ans cuve + 2 ans pièces.

### 📄 Contrat entretien chauffe-eau
| | TTC |
|--|----|
| Contrat annuel (détartrage cuve + vérif groupe sécurité + anode + joints) | **220 €** |

## 🔥 Chauffage (3 forfaits + 6 contrats)

| Prestation | Tarif TTC |
|------------|-----------|
| Entretien annuel chaudière gaz | **121 €** |
| Désembouage par radiateur | **105 €** |
| Dépannage T1 (semaine, sans fourniture) | **75 €** |

### Contrats annuels
| Formule | Gaz | Fioul |
|---------|-----|-------|
| **Basic** | 9 €/mois (108 €/an) | 12 €/mois (140 €/an) |
| **Confort** | 13 €/mois (156 €/an) | 16 €/mois (190 €/an) |
| **Sécurité** | 23 €/mois (276 €/an) | 27 €/mois (320 €/an) |

### Poêles et inserts — montants **HT** (barème agence)
> Source : barème agence, `BAREME AGENCE > PLOMBERIE > FORFAIT & MAIN D'OEUVRE > CHAUFFAGE`, transmis par Florian le 18/09/2026 (directive 5733225819). Montants **HT** : ne pas les afficher comme des TTC.

| Code | Prestation | Tarif HT | TTC particulier, logement de plus de 2 ans (TVA 10 %) | TTC autres cas (TVA 20 %) |
|------|------------|----------|-------------------------------------------------------|---------------------------|
| **EPB** | Entretien annuel poêle / insert **à bois** — **ramonage compris** | **115 € HT** | 126,50 € | 138,00 € |
| **EPG** | Entretien annuel poêle / insert **à granulés** — **ramonage compris** | **136 € HT** | 149,60 € | 163,20 € |

> Catalogue en ligne : lignes prêtes dans `supabase/_pending_migrations/20260919100000_catalogue_entretien_poele_insert.sql` (115 € HT et 136 € HT, TVA 10 %, affichées « TTC · TVA 10 % » avec la bascule Pro à 20 %). **Non appliquées** : décision de Florian le 22/09/2026.

Règle TVA (CGI art. 279-0 bis) : 10 % pour des travaux d'entretien dans un logement achevé depuis plus de 2 ans, 20 % sinon (logement récent, local professionnel). Affichage public : le HT et les deux TTC, jamais un TTC unique présenté comme valable pour tous.
Ces deux tarifs ne s'appliquent **qu'aux poêles et inserts** (bois / granulés). Ramonage de cheminée seul ou de conduit seul : **pas de tarif confirmé** (« sur devis »).
Catalogue en ligne (table `services`) : ajout préparé dans `supabase/_pending_migrations/20260919100000_catalogue_entretien_poele_insert.sql`, **non appliqué** (écriture en base PROD = GO Florian).

## ⚡ Électricité (2 forfaits)

| Prestation | Tarif TTC |
|------------|-----------|
| Recherche de panne électrique | **107 €** |
| Forfait T1 panne sans fourniture (semaine) | **75 €** |

## 🔒 Serrurerie (4 forfaits)

| Prestation | Tarif TTC |
|------------|-----------|
| Ouverture porte simple | **108 €** |
| Ouverture porte claquée | **176 €** |
| Ouverture porte fermée à clé | **228 €** |
| Ouverture porte sécurité | **314 €** |

## 🪟 Vitrerie

| Prestation | Tarif TTC |
|------------|-----------|
| Mise en sécurité ouverture extérieur | **120 €/m²** |
| Réalisation gabarit | **184 €** |

---

## ❓ En attente de validation Florian

À compléter à son retour (prix à donner) :

**Plomberie**
- Pose chauffe-eau (par tranche de litres)
- Remplacement chauffe-eau à l'identique
- Pose WC suspendu / au sol neuf
- Réparation/remplacement mitigeur semaine
- Réparation fuite simple (raccord, joint)

**Chauffage**
- Entretien chaudière FIOUL
- Vidange + remplissage installation
- Remplacement vase d'expansion
- Remplacement circulateur

**Électricité**
- Remplacement disjoncteur seul
- Remplacement prise / interrupteur
- Pose luminaire simple
- Diagnostic électrique avant vente/location

**Serrurerie**
- Changement cylindre standard

**Vitrerie**
- Remplacement vitre simple (selon dimensions)

---

## 🚫 Toujours en "sur devis"

- Tableau électrique complet (sauf si client veut tarif à partir de)
- Pompe à chaleur (PAC)
- Rénovation salle de bain
- Rénovation cuisine
- Adaptation PMR globale
- Blindage de porte
- Travaux tous corps d'état

---

## 🛡️ Règle pour l'agent IA

Tout montant `\d+\s*€` affiché sur une page publique doit :
1. Apparaître dans ce document, OU
2. Être marqué clairement "estimation marché" / "exemple non engageant", OU
3. Avoir l'attribut HTML `data-source="base-produits-2026-05"`

Sinon → sonde IA #23 alerte rouge.

---

*Document généré automatiquement le 15/05/2026 — toute modif doit être validée par Florian.*
