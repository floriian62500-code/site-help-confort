# Audit FAQPage doublon — 2026-05-20 06:52

- **Pages scannées** : 81
- **Pages avec 1 bloc FAQPage** : 50
- **Pages sans FAQPage** : 31
- **Pages avec > 1 bloc FAQPage (alertes)** : 0

> Google recommande un seul bloc `FAQPage` par URL. Au-delà, le rich result peut être ignoré silencieusement.

## ✅ Aucun doublon FAQPage détecté

## Pages avec FAQPage (1 bloc, OK)

- `aides.html` (2 blocs JSON-LD au total)
- `chauffagiste-boulogne-sur-mer.html` (5 blocs JSON-LD au total)
- `chauffagiste-calais.html` (5 blocs JSON-LD au total)
- `chauffagiste-dunkerque.html` (5 blocs JSON-LD au total)
- `chauffagiste-saint-omer.html` (5 blocs JSON-LD au total)
- `contrats-entretien.html` (3 blocs JSON-LD au total)
- `debouchage-canalisation.html` (6 blocs JSON-LD au total)
- `depannage-arques.html` (3 blocs JSON-LD au total)
- `depannage-bergues.html` (3 blocs JSON-LD au total)
- `depannage-boulogne-sur-mer.html` (4 blocs JSON-LD au total)
- `depannage-calais.html` (4 blocs JSON-LD au total)
- `depannage-coquelles.html` (3 blocs JSON-LD au total)
- `depannage-dunkerque.html` (3 blocs JSON-LD au total)
- `depannage-gravelines.html` (3 blocs JSON-LD au total)
- `depannage-longuenesse.html` (3 blocs JSON-LD au total)
- `depannage-saint-martin-lez-tatinghem.html` (3 blocs JSON-LD au total)
- `depannage-saint-omer.html` (3 blocs JSON-LD au total)
- `depannage-saint-pol-sur-mer.html` (3 blocs JSON-LD au total)
- `depannage-sangatte.html` (3 blocs JSON-LD au total)
- `diagnostic-electrique.html` (4 blocs JSON-LD au total)
- `electricien-boulogne-sur-mer.html` (5 blocs JSON-LD au total)
- `electricien-calais.html` (5 blocs JSON-LD au total)
- `electricien-dunkerque.html` (5 blocs JSON-LD au total)
- `electricien-saint-omer.html` (5 blocs JSON-LD au total)
- `entretien-chaudiere.html` (5 blocs JSON-LD au total)
- `faq.html` (2 blocs JSON-LD au total)
- `menuisier-dunkerque.html` (5 blocs JSON-LD au total)
- `menuisier-saint-omer.html` (5 blocs JSON-LD au total)
- `ouverture-porte-claquee.html` (6 blocs JSON-LD au total)
- `panne-chaudiere.html` (4 blocs JSON-LD au total)
- `plombier-boulogne-sur-mer.html` (5 blocs JSON-LD au total)
- `plombier-calais.html` (5 blocs JSON-LD au total)
- `plombier-dunkerque.html` (5 blocs JSON-LD au total)
- `plombier-saint-omer.html` (5 blocs JSON-LD au total)
- `pmr-dunkerque.html` (4 blocs JSON-LD au total)
- `pmr-saint-omer.html` (4 blocs JSON-LD au total)
- `remplacement-chauffe-eau.html` (6 blocs JSON-LD au total)
- `serrurier-boulogne-sur-mer.html` (5 blocs JSON-LD au total)
- `serrurier-calais.html` (5 blocs JSON-LD au total)
- `serrurier-dunkerque.html` (5 blocs JSON-LD au total)
- `serrurier-saint-omer.html` (5 blocs JSON-LD au total)
- `tarifs.html` (4 blocs JSON-LD au total)
- `travaux-dunkerque.html` (4 blocs JSON-LD au total)
- `travaux-saint-omer.html` (4 blocs JSON-LD au total)
- `urgence.html` (3 blocs JSON-LD au total)
- `vitrier-dunkerque.html` (5 blocs JSON-LD au total)
- `vitrier-saint-omer.html` (5 blocs JSON-LD au total)
- `volets-dunkerque.html` (5 blocs JSON-LD au total)
- `volets-saint-omer.html` (5 blocs JSON-LD au total)
- `zones-intervention.html` (5 blocs JSON-LD au total)
