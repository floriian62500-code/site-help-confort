...
- [ ] **Sonde `audit_meta_description_length.py`** — détecter les `<meta name="description">` < 70 ou > 160 caractères (SEO Google : 120-158 idéal). Rapport `.md` + `.json`. À ajouter au workflow nightly.
- [ ] **Sonde `audit_h1_length.py`** — détecter les `<h1>` < 20 ou > 70 caractères (SEO + UX). Tolérer pages dynamiques type `realisation.html`. Rapport `.md`.
- [ ] **Sonde `audit_image_dimensions.py`** — étendre `audit_cls_prevention.py` : pour chaque `<img>` sans `width`/`height`, lire les vraies dimensions avec PIL (si fichier local) et proposer le patch HTML correct dans le rapport. Décision Florian sur application masse.
- [ ] **Sonde `audit_phone_consistency.py`** — extraire tous les `tel:+33...` et numéros affichés (`03 66 10 01 34`, etc.) du site et vérifier qu'ils sont tous identiques au numéro canonique TARIFS_REFERENCE / mentions légales. Tout numéro orphelin → ALERTE.
- [ ] **Sonde `audit_robots_sitemap_url.py`** — vérifier que `robots.txt` contient bien la directive `Sitemap: https://www.depan59-62.fr/sitemap.xml` (Google Search Console recommandation).
- [ ] **Sonde `audit_inline_style_size.py`** — détecter les pages avec > 50 KB de CSS inline (`<style>...</style>` cumulé). Suggère extraction vers `styles.css`. Top 10 pages dans le rapport.
- [ ] **Sonde `audit_admin_pro_isolation.py`** — vérifier qu'aucune page `admin-pro/*.html` n'est référencée depuis une page publique racine (sécurité : l'admin doit rester accessible uniquement via login direct).
- [ ] **Sonde `audit_consent_default_state.py`** — vérifier que `assets/hc-consent.js` ne dépose AUCUN cookie/localStorage tant que le user n'a pas cliqué Accept (RGPD strict). Parse le JS et alerte sur tout `document.cookie =` ou `localStorage.setItem(` exécuté avant le clic Accept.
