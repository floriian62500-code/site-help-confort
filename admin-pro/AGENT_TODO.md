...
- [ ] **Sonde #42 — Audit Leaflet map init** — créer `admin-pro/audits/audit_leaflet_init.py` qui détecte les pages contenant `L.map(` ou `new L.Map(` et vérifie la présence d'un `setTimeout(() => map.invalidateSize(), ...)` OU `window.addEventListener('load', ...)` après init. Sans cela → ALERTE *carte vide possible*.
- [ ] **Workflow nightly v2** — étendre `.github/workflows/audit.yml` avec les nouveaux audits P12 (`audit_sitemap_completeness`, `audit_heading_hierarchy`, `audit_og_image`, `audit_sw_version`, `audit_fallback_supabase`, `audit_typo_fr`, `audit_faqpage_doublon`, `audit_schema_type_metier`, `audit_leaflet_init`). Maintenir la cascade `continue-on-error` + `digest_mensuel.py` en bout.
