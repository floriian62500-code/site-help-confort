...
- [ ] **Sonde #47 — Audit SW version freshness** — créer `admin-pro/audits/audit_sw_version.py` qui parse `sw.js` pour récupérer `const VERSION = '...'` et croise avec la date du dernier commit ayant modifié un asset (`images/`, `assets/`, `og/`). Si VERSION < 7 jours avant le dernier commit asset → ALERTE *SW cache obsolète*. Rapport `.md`.
- [ ] **Sonde #49 — Audit fallback Supabase** — créer `admin-pro/audits/audit_fallback_supabase.py` qui parse les pages publiques contenant un appel `supabase.from(` ou `fetch.../rest/v1/`, et vérifie la présence d'un `LOCAL_CATALOG` / `FALLBACK_` / `FALLBACK_DATA` hardcodé. Sans fallback → ALERTE *page vide si coupure Supabase*. Rapport `.md`.
- [ ] **Sonde #52 — Audit typographie FR (`&nbsp;` insécables)** — créer `admin-pro/audits/audit_typo_fr.py` qui compte les `?!:;»` non précédés d'un `&nbsp;` ou espace insécable dans les pages publiques. Seuil : > 5 → ALERTE *typo FR non conforme*. Whitelist `<code>`, `<pre>`, `<script>`, `<style>`. Rapport `.md`.
