...
- [ ] **Sonde #31 v5 — Audit FAQPage doublon** — créer `admin-pro/audits/audit_faqpage_doublon.py` qui compte les `<script type="application/ld+json">` typés `FAQPage` par page. Si > 1 → ALERTE (Google recommande un seul bloc FAQPage par URL). Rapport `.md`.
- [ ] **Sonde #29 v4 — Audit Schema `@type` métier** — créer `admin-pro/audits/audit_schema_type_metier.py` qui pour chaque page `<métier>-saint-omer.html` croise `@type` du JSON-LD principal avec le mapping attendu : plombier→Plumber, chauffagiste→HVACBusiness, electricien→Electrician, serrurier→Locksmith, travaux→GeneralContractor. Tout mismatch → ALERTE. Rapport `.md`.
