-- ═══════════════════════════════════════════════════════════════════════════
-- Insertion de 3 actus adaptées des publications officielles HC France
-- Date : 2026-05-16
-- ═══════════════════════════════════════════════════════════════════════════
-- 1. Vitrerie — double vitrage (calme, fraîcheur, économies)
-- 2. Plomberie — chasse d'eau fuit (600L/jour gaspillés)
-- 3. Électricité — éclairage printemps (modernisation luminaires)
-- ═══════════════════════════════════════════════════════════════════════════
...
(
  ...
  jsonb_build_object(
    ...
    'inspired_by', 'HC France — double vitrage',
    'hashtags', array['doublevitrage','isolationthermique','saintomer','dunkerque'],
...
(
  ...
  jsonb_build_object(
    'post_type', 'actualite',
    'imported_from', 'hc_fr_manual_adapted',
    'inspired_by', 'HC France — chasse d''eau fuite',
...
(
  ...
  jsonb_build_object(
    'post_type', 'actualite',
    'imported_from', 'hc_fr_manual_adapted',
    'inspired_by', 'HC France — éclairage printemps',
    'hashtags', array['electricite','eclairage','led','saintomer','dunkerque'],
