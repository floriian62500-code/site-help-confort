...
ok('famille : entretien chaudière (prestation au panier ou contrat), entretien poêle / insert (prestations du barème EPB / EPG), ramonage par la page d’atterrissage ; le reste → aucune', C.serviceFamily({ slugs: ['entretien-chaudiere-gaz'] }) === 'chaudiere' && C.serviceFamily({ slugs: ['entretien-chaudiere-fioul-gros-volume'] }) === 'chaudiere' && C.serviceFamily({ metiers: ['Contrat entretien'] }) === 'chaudiere' && C.serviceFamily({ slugs: ['entretien-poele-insert-bois'] }) === 'poele' && C.serviceFamily({ slugs: ['entretien-poele-insert-granules'] }) === 'poele' && C.serviceFamily({ slugs: ['devis-vitre-insert-poele'] }) === null && C.serviceFamily({ src: 'ramonage' }) === 'ramonage' && C.serviceFamily({ src: 'poele-insert' }) === null && C.serviceFamily({ slugs: ['debouchage-canalisation'] }) === null);
