...
echo "— POSTFLIGHT —"
STAT="$SB/status.json"; echo '{"heartbeat":"2026-01-01T00:00:00Z","state":"SITE_WORK_ACTIVE","last_report":"docs/control/outbox/claude/RUN-x.md"}' > "$STAT"
...
# 8. cas nominal OK
checkpost "nominal OK" "OK postflight" POST_BRANCH=recette POST_REMOTE="$OKREM" POST_STATUS="$STAT" POST_CHANGED="$OK_CHANGED"
# 9. tentative modif workflow => FAIL
checkpost "modif garde-fou (workflow)" "FAIL diff touche un garde-fou" POST_BRANCH=recette POST_REMOTE="$OKREM" POST_STATUS="$STAT" POST_CHANGED=$'.github/workflows/claude-runner-oauth.yml\ndocs/control/outbox/claude/RUN-x.md\ndocs/control/runner-status.json'
# 10. absence outbox => FAIL
checkpost "outbox manquant" "FAIL aucun outbox" POST_BRANCH=recette POST_REMOTE="$OKREM" POST_STATUS="$STAT" POST_CHANGED='docs/control/runner-status.json'
# 11. runner-status JSON invalide => FAIL
BADSTAT="$SB/bad.json"; echo '{invalid' > "$BADSTAT"
checkpost "runner-status JSON invalide" "FAIL runner-status.json invalide" POST_BRANCH=recette POST_REMOTE="git@github.com:x/site-help-confort.git" POST_STATUS="$BADSTAT" POST_CHANGED="$OK_CHANGED"
# 12. branche != recette => FAIL
checkpost "branche != recette" "FAIL branche != recette" POST_BRANCH=main POST_REMOTE="git@github.com:x/site-help-confort.git" POST_STATUS="$STAT" POST_CHANGED="$OK_CHANGED"
# 13. remote origin altéré => FAIL
checkpost "remote origin inattendu" "FAIL remote origin inattendu" POST_BRANCH=recette POST_REMOTE="git@github.com:attaquant/autre-repo.git" POST_STATUS="$STAT" POST_CHANGED="$OK_CHANGED"
# 14. runner-status champs manquants (heartbeat/state/last_report) => FAIL
INCOH="$SB/incoherent.json"; echo '{"foo":1}' > "$INCOH"
checkpost "runner-status champs manquants" "FAIL runner-status.json incohérent" POST_BRANCH=recette POST_REMOTE="git@github.com:x/site-help-confort.git" POST_STATUS="$INCOH" POST_CHANGED="$OK_CHANGED"
