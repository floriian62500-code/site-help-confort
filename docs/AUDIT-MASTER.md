...
| A17 | domaine canonique (sitemap + `<link canonical>`) | SEO | Site servi sur **apex** `depan59-62.fr` (www→301→apex) mais sitemap & canonical déclarent **`www.`** → conflit canonique (URLs en 301 au lieu de 200 direct) | P2 | Aligner sur l'apex : `SITE_URL` edge sitemap + canonicals des pages. Edge = **backend prod (gate)** | — | — | ⏳ | — | — |
...
| A21 | Centre de validation `/recette.html` | Process | Validation Florian trop technique (SHA/PR/branche) | P1 | Page simple : cartes par page, VOIR/OK/À CORRIGER + commentaire (persistés Supabase `recette_validation`), progression, « Valider pour prod » (enregistre la décision, ne déploie pas). Gated recette (masqué en prod). Claude relit les retours en base | ✅ E2E : rendu + persistance OK/KO+commentaire prouvés en base | ✅ | ⏳ | — | — |
