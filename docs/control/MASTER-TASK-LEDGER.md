...
| CMD-5 | 5438316639 | P1 | Retrait complet ancien sélecteur unique (proposePrestations/detectDetailedPresta/renderPrestaProposals) | **BACKLOG — bloqué par preuve 0-réf FAUSSE** | — | analyse réf faite | — | **NON inerte** : buildResult() branche `simple` (index.html l.2235-2293) exécute encore `detectDetailedPresta()` (l.2237) + `renderPrestaProposals()` (l.2273/2281/2291) vers stubs cachés ; `resa-presta-name-c` sert aussi le récap. Suppression aveugle = ReferenceError → wizard cassé. **PRÉREQUIS = UX-C-e** (migrer branche `simple` + récap confirmation sur le panier `hc-cart.js`) PUIS suppression. Ne pas supprimer avant. |
