...
| CAT-D1 | 5452903372 pt5 (lot D) | P0 | Reprise catalogue refusé (CMD-8) — incrément 1 : icônes familles cassées + emoji | **READY_FOR_QA** | `ed6b9659` | **public preview** : cartes familles affichaient noms Lucide en texte (Wrench/Thermometer/Zap/Lock/Square) → `famIcon()` mappe → **vrais SVG** (6/6, 0 texte) ; cartes premium (icône dégradé+nom+compteur+chevron) catalogue+diagnostic ; panier 🗑→SVG + contexte ⏱📍🏢→SVG (**0 emoji**) ; multi-panier/qty/total prouvés (ajout ×2, qty 2→3, total 783 TTC) ; diagnostic 6 familles→15 problèmes ; **QA publique 1440 + 2ᵉ inspection (emoji trouvé→corrigé)** | `MULTICART/FIXED_PRICE/DIAGNOSTIC/BACK_NAV/CART_PERSISTENCE=PASS · PUBLIC_PREVIEW_VISUAL=PROVEN · SELF_ITERATIONS=1` | **incrément** de lot D (reste : profondeur esthétique refonte globale si Florian précise) ; validation visuelle Florian |
| DEVIS-2 | 5452961398 / 5453065046 | P0 | Devis REFUSED_VISUAL (trop administratif) → **wizard progressif centré premium** | **READY_FOR_QA** | `2ec99d5b`,`d885022f` | **QA preview PUBLIQUE** : 5 étapes (Besoin cartes-pictos métier / Demande / Lieu+**agence auto**+**autocomplete BAN** / Coordonnées / **Vérification récap + Modifier par bloc**) → confirmation dédiée ; conteneur **centré** (panneau latéral retiré pour devis) ; stepper numéroté actif/complété ; micro-rassurance intégrée ; SVG (0 emoji) ; **1440 centré** + **390 tunnel natif 2-col cartes tactiles** + step5 recap/CTA fort ; back-nav **sans perte** (services+desc+cp conservés) ; **soumission test réelle → dossier backend** ; console 0 | backend **INCHANGÉ** (form_type=devis_express, colonnes standards, services[]) ; remplace DEVIS-1 | validation Florian |
| DEVIS-1 | 5451445138 | P0 | Formulaire devis dédié (v1, refusé visuellement → refait en wizard DEVIS-2) | **SUPERSEDED** | `e84f7bf8` | navigateur 1440 (2 col form 788 + réassurance 340) + 390 (stack, bande confiance) ; 8 champs standards + 12 services multi + description ; validation inline + focus 1er invalide + garde double-clic ; **soumission test réelle → dossier backend `48a48db3…` créé** (form_type=devis_express, colonnes standards + services[]) ; confirmation dédiée « Demande de devis envoyée » + récap ; pièces jointes UX honnête ; 0 overflow ; console 0 | `QUOTE_FORM_UI · REQUIRED_CONTACT_FIELDS · ADDRESS · MULTI_SERVICE · DESCRIPTION · VALIDATION · SUBMIT · EMAIL_TYPE=DEMANDE_DEVIS · BACKOFFICE_FIELDS · CONFIRMATION · MOBILE · OTHER_FORMS=NO_REGRESSION = PASS` ; email `notify-lead-v6` devis dédié = amélioration edge future (gate) | validation Florian |
...
| ENT-2 | 5451658420 pt2 | P0 | Tuile `Entretien & contrats` du launcher `/catalogue` = découverte → doit aussi pointer `/contrats-entretien` (ChatGPT: OUI) | **READY_FOR_QA** | `e001b31c` | navigateur : tuile = `<a href="/contrats-entretien.html">` ; **clic réel** → page contrats (formules) ; anchor non intercepté (0 data-step-go) ; text-decoration:none ; audit : **0** autre entrée découverte `#step=entretien` restante ; tunnel entretien conservé pour conversion | pas de boucle ; discovery→contrats, conversion(post-formule)→tunnel | validation Florian |
...
| TRD-2 | QA-observée (5451558329) | P1 | RSP-1 résiduel : header ancien pages métiers masque n° tél seulement ≤979px → overflow 980-1100 | **READY_FOR_QA (sweep complet 138 pages)** | `11431255`,`6185eda8` | navigateur 1024 : overflow 1098→0, n° tél masqué ≤1100 (bouton icône conservé) ; 7 pages | anomalie observée pendant QA du bloc engagements, corrigée atomiquement | validation Florian ; **sweep généralisé FAIT (131 pages + 7) ; 2 pages header partagé sans overflow (aucun correctif) |
...
| CTR-1 | 5453208485 (P0) / 5453180213 | P0 | Souscription contrat : faux succès + erreur RLS brute + photo bloquante | **READY_FOR_QA** (front) ; **row `contracts` = GATE** | `22e98c62` | cause : double soumission (insert anon `contracts`=viole RLS→erreur SQL rouge + data-hc-lead→submit-lead-v6→succès vert) ; fix = retrait data-hc-lead + route via submit-lead-v6 (service_role) + succès si resp.ok seul + 0 erreur brute + photos facultatives ; **preview PUBLIQUE Gaz BASIC 0 photo : confirmation unique, 0 erreur RLS** | `docs/control/CONTRACT-RLS-GATE.md` : row `contracts` via edge `submit-contract` service_role (option A) OU policy INSERT bornée (B) = `CONTRACT_RLS_FIX=BLOCKED_HUMAN_GATE` | GO Florian (deploy edge/apply) |
| CTR-PHOTO | 5453154027 | P0 | Upload photo réel dans le wizard devis (Storage) | **BLOCKED_HUMAN_GATE** (architecture) | — | nécessite bucket Storage privé + RLS + edge signed-url + lien lead ; pas de faux input livré (UX honnête « pas encore dispo ») | `PHOTO_UPLOAD=BLOCKED_HUMAN_GATE` : concevoir chemin E2E (bucket privé non listable, MIME/taille serveur, liens signés, rattachement dossier) | GO Florian (Storage/RLS/edge) |
